	...
	else
	{
		...
		else
		{
			...
			if (travelling_at_hyperspeed)
			{
				...
				// Должна быть вычисляемой величиной от коэффициента масштабирования (наверно квадратичная зависимость)
...
- (BOOL) checkEntityForMassLock:(Entity *)ent withScanClass:(int)theirClass
{
	...
	if (EXPECT_NOT([ent isStellarObject]))
	{
		...
		if (EXPECT([stellar planetType] != STELLAR_TYPE_MINIATURE))
		{
			...
			double factor = ([stellar isSun]) ? 1.2 : 1.0; 
			...
			if (dist < rad*rad*factor*factor || dist < rad*rad + 50000*rad + 625000000 ) 
			{
				massLocked = YES;
			}
		}
